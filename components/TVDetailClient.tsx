...
import { useTVDetailQuery } from "@/features/detail/queries";
...
export function TVDetailClient({ id }: TVDetailClientProps) {
	const { data: show, isLoading, isError, error } = useTVDetailQuery(id);

	if (isLoading) {
		return <div className="min-h-screen page-shell pb-16" />;
	}
		...
		return (
			<div className="min-h-screen page-shell pb-16">
				<main className="max-w-3xl mx-auto px-4 pt-20">
					<div className="glass-panel p-6 text-center text-slate-100">
						<p className="text-lg font-semibold text-white">Unable to load TV details.</p>
