			...
			<div className="max-w-7xl mx-auto px-4 py-6">
				<div className="space-y-4 glass-panel p-4 md:p-5">
					...
					<div className="flex flex-wrap gap-2 items-center">
						<div className="flex gap-2">
							{(["all", "movie", "tv"] as const).map((t) => (
								<button
									...
									className={`px-4 py-2 rounded-full text-[13px] md:text-sm font-semibold transition-all duration-200 border ${
						...
						<label className="max-md:hidden ml-auto flex items-center gap-2 text-[10px] md:text-sm text-slate-200">
							Year
							<input
								type="number"
								min={1900}
						...
						<label className="md:hidden ml-auto flex items-center gap-2 text-[10px] md:text-sm text-slate-200">
							<input
								type="number"
								min={1900}
								max={2100}
								...
								onChange={(e) => updateYear(e.target.value)}
								placeholder="Year"
								className="w-24 rounded-full bg-white/10 border border-white/20 px-3 py-2 text-sm text-white text-center placeholder-slate-300/60 focus:outline-none focus:ring-2 focus:ring-cyan-300/35"
							/>
						</label>
